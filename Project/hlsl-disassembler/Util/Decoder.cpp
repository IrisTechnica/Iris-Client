...
uint32_t HLSLDisassembler::GenerateMask(uint8_t start, uint8_t end)
{
	uint32_t mask = 0;
...
uint32_t HLSLDisassembler::DecodeValue(uint32_t token, uint8_t start, uint8_t end)
{
...
int8_t HLSLDisassembler::DecodeSigned4BitValue(uint32_t token, uint8_t start, uint8_t end)
{
	if (end - start != 3)
...
string HLSLDisassembler::ToFourCcString(uint32_t fourCc)
{
	char chars[4];
